...
describe('remix-example', () => {
  ...
  it('remix should generate CSS with expected styles for .main class', async () => {
    buildExample(exampleDir, 'npm run build')
    const css = getCSSFromExample(exampleDir, 'build/client/assets')
  ...
  it('remix should generate CSS with expected styles for .card class', async () => {
    const css = getCSSFromExample(exampleDir, 'build/client/assets')
    expect(css).toBeTruthy()
